CREATE TABLE IF NOT EXISTS oulad.oulad_gold.fact_assessment (
    ...
    date_submitted_key BIGINT NOT NULL,
    score INT,
    is_banked INT,
...
USING (
    SELECT
        ds.student_key,
        da.course_key,                          -- ✅ resolved via dim_assessment
        da.assessment_key,
        dd.date_key AS date_submitted_key,
        sa.score,
        sa.is_banked,
    ...
    JOIN oulad.oulad_gold.dim_student ds
        ON sa.id_student = ds.id_student        -- ✅ only natural key available
    JOIN oulad.oulad_gold.dim_assessment da
        ON sa.id_assessment = da.id_assessment  -- ✅ resolve course_key + assessment_key here
...
ON tgt.student_key = src.student_key
   ...
   AND tgt.date_submitted_key = src.date_submitted_key
WHEN MATCHED THEN
    UPDATE SET
        tgt.score = src.score,
...
WHEN NOT MATCHED THEN
    INSERT (student_key, course_key, assessment_key, date_submitted_key, score, is_banked, gold_processed_timestamp, gold_processed_date)
    VALUES (src.student_key, src.course_key, src.assessment_key, src.date_submitted_key, src.score, src.is_banked, src.gold_processed_timestamp, src.gold_processed_date);
